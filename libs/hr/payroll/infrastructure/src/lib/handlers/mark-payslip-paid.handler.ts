...
import { MarkPayslipPaidCommand } from '@abms/hr-payroll-application';
...
@Injectable()
@CommandHandler(MarkPayslipPaidCommand)
export class MarkPayslipPaidHandler extends TransactionalCommandHandler<MarkPayslipPaidCommand, void> {
  public constructor(
  ...
  protected async handle(command: MarkPayslipPaidCommand, ctx: ITransactionContext): Promise<void> {
    ...
    const paidByUserId = this.currentUserProvider.getCurrentUserId() ?? 'unknown';
    payslip.markPaid(paidByUserId);
