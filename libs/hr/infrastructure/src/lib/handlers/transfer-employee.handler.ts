...
import { TransferEmployeeCommand } from '@abms/hr-application';
...
@Injectable()
@CommandHandler(TransferEmployeeCommand)
export class TransferEmployeeHandler extends TransactionalCommandHandler<TransferEmployeeCommand, void> {
  public constructor(
  ...
  protected async handle(command: TransferEmployeeCommand, ctx: ITransactionContext): Promise<void> {
    ...
    employee.transferToOrgUnit(command.newOrgUnitId);
    ...
    await historyRepository.append(
      EmploymentHistoryEntry.create({
        ...
        details: `Transferred from org unit "${previousOrgUnitId ?? 'none'}" to "${command.newOrgUnitId ?? 'none'}".`,
      }),
    );
  }
}
