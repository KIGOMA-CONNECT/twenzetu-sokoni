...
- [ ] **Design system (Part B)** — token + component proposal shipped ([`docs/design/design-system.md`](design/design-system.md)): spacing/motion tokens and `prefers-reduced-motion` applied to `globals.css`; pending component sweep (buttons/inputs/cards/toasts/skeletons), inline-style refactor, image treatment, dark mode.
