...
| Analytics | **L3** | Metrics exposed (`/api/metrics`) + monitoring stack; tenant-facing reports (vendor + admin consoles: sales summary, daily series, order funnel, customer acquisition, delivery performance, top products, inventory, metric catalog) with CSV export and a documented metric catalog (`docs/analytics/metric-catalog.md`). Next: self-service dashboards → L4. |
...
- [ ] **Cargo, Express & Logistics L5** — driver app offline mode, delivery SLA dashboards, bulk fleet ops. (L4 shipped: driver earnings, capacity dispatch, ETA, delivery + pickup OTP, consumer driver ratings.)
