    ...
    <div>
      ...
      {tab === 'revisions' && <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
          <thead><tr style={{ background: 'var(--bg)' }}><th style={{ padding: '0.6rem', textAlign: 'left' }}>Employee</th><th style={{ padding: '0.6rem', textAlign: 'right' }}>Previous</th><th style={{ padding: '0.6rem', textAlign: 'right' }}>New</th><th style={{ padding: '0.6rem', textAlign: 'left' }}>Reason</th><th style={{ padding: '0.6rem', textAlign: 'left' }}>Effective</th></tr></thead>
      ...
      {tab === 'benefits' && <div>
        ...
        {showPlanForm && <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', padding: '1rem', background: 'var(--bg)', borderRadius: '8px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Name *</div><input style={s.input} value={pf.name} onChange={e => setPf(f => ({ ...f, name: e.target.value }))} /></div>
          <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Type</div><select style={s.input} value={pf.type} onChange={e => setPf(f => ({ ...f, type: e.target.value }))}><option value="MEDICAL">Medical</option><option value="DENTAL">Dental</option><option value="LIFE">Life</option><option value="PENSION">Pension</option><option value="OTHER">Other</option></select></div>
          <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Employer</div><input type="number" style={{ ...s.input, width: '100px' }} value={pf.employerContribution} onChange={e => setPf(f => ({ ...f, employerContribution: +e.target.value }))} /></div>
          <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Employee</div><input type="number" style={{ ...s.input, width: '100px' }} value={pf.employeeContribution} onChange={e => setPf(f => ({ ...f, employeeContribution: +e.target.value }))} /></div>
        ...
        <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: '1rem' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            ...
            <tbody>{plans?.map(p => <tr key={p.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
              ...
              <td style={{ padding: '0.6rem', textAlign: 'center' }}><span style={{ background: p.isActive ? 'var(--success-soft)' : 'var(--danger-soft)', color: p.isActive ? 'var(--success)' : 'var(--danger)', padding: '0.1rem 0.4rem', borderRadius: '999px' }}>{p.isActive ? 'Active' : 'Inactive'}</span></td>
        ...
        <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', alignItems: 'flex-end' }}>
            ...
            <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Plan</div><select style={s.input} value={enrollForm.benefitPlanId} onChange={e => setEnrollForm(f => ({ ...f, benefitPlanId: e.target.value }))}><option value="">Select...</option>{plans?.filter(p => p.isActive).map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
            <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Coverage Start</div><input type="date" style={s.input} value={enrollForm.coverageStart} onChange={e => setEnrollForm(f => ({ ...f, coverageStart: e.target.value }))} /></div>
          ...
          {selEmpId && selEnrollments && <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead><tr style={{ background: 'var(--bg)' }}><th style={{ padding: '0.5rem', textAlign: 'left' }}>Plan</th><th style={{ padding: '0.5rem', textAlign: 'center' }}>Status</th><th style={{ padding: '0.5rem', textAlign: 'center' }}>Actions</th></tr></thead>
            <tbody>{selEnrollments.map(e => <tr key={e.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
              ...
              <td style={{ padding: '0.5rem', textAlign: 'center' }}>{e.status === 'ACTIVE' && <button style={{ ...s.btn, background: 'var(--warning)' }} onClick={() => cancelEnroll(e.id)}>Cancel</button>}</td>
            </tr>)}</tbody>
          </table>}
        </div>
      </div>}
      ...
      {tab === 'structures' && <div>
        ...
        {selEmpId && <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '1rem' }}>
          {selStructures && selStructures.length > 0 ? (
            ...
            <div>
              <p style={{ color: 'var(--muted)', marginBottom: '1rem' }}>No salary structure yet. Create one:</p>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '0.5rem', marginBottom: '1rem' }}>
                ...
                <div><div style={{ fontSize: '0.7rem', color: 'var(--muted)' }}>Currency</div><select style={s.input} value={strucForm.currency} onChange={e => setStrucForm(f => ({ ...f, currency: e.target.value }))}><option value="RWF">RWF</option><option value="TZS">TZS</option><option value="USD">USD</option></select></div>
              </div>
