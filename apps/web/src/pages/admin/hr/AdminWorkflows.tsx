  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.4rem', marginBottom: '1rem' }}>
        <button onClick={() => setTab('definitions')} style={{ padding: '0.4rem 0.8rem', borderRadius: '5px', border: tab === 'definitions' ? '2px solid #3b82f6' : '1px solid #cbd5e1', background: tab === 'definitions' ? '#eff6ff' : '#fff', fontWeight: 500, color: tab === 'definitions' ? '#3b82f6' : '#475569' }}>Definitions</button>
        <button onClick={() => setTab('instances')} style={{ padding: '0.4rem 0.8rem', borderRadius: '5px', border: tab === 'instances' ? '2px solid #3b82f6' : '1px solid #cbd5e1', background: tab === 'instances' ? '#eff6ff' : '#fff', fontWeight: 500, color: tab === 'instances' ? '#3b82f6' : '#475569' }}>Instances</button>
      </div>
      ...
      {tab === 'definitions' && <div>
        ...
        {showForm && <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', padding: '1rem', background: '#f8fafc', borderRadius: '8px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
          ...
          <div style={{ width: '160px' }}><div style={{ fontSize: '0.75rem', color: '#64748b' }}>Entity Type *</div><input style={s.input} value={f.entityType} onChange={e => setF(p => ({ ...p, entityType: e.target.value }))} placeholder="e.g. leave_request" /></div>
        ...
        <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead><tr style={{ background: '#f8fafc' }}><th style={{ padding: '0.6rem', textAlign: 'left' }}>Name</th><th style={{ padding: '0.6rem', textAlign: 'left' }}>Entity</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Steps</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Status</th></tr></thead>
            <tbody>{defs?.map(d => <tr key={d.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
              ...
              <td style={{ padding: '0.6rem', color: '#64748b' }}>{d.entityType}</td>
              <td style={{ padding: '0.6rem', textAlign: 'center' }}>{d.steps?.length || 0}</td>
              <td style={{ padding: '0.6rem', textAlign: 'center' }}><span style={{ background: d.isActive ? '#dcfce7' : '#fef2f2', color: d.isActive ? '#16a34a' : '#dc2626', padding: '0.1rem 0.4rem', borderRadius: '999px' }}>{d.isActive ? 'Active' : 'Inactive'}</span></td>
            </tr>)}
          </tbody>
          </table>
          {expandedDef && defs?.find(d => d.id === expandedDef) && <div style={{ padding: '0.75rem', borderTop: '2px solid #e2e8f0', background: '#f8fafc' }}>
            ...
            {(defs.find(d => d.id === expandedDef)?.steps || []).map((step, i) => <div key={step.id} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.3rem 0', fontSize: '0.85rem' }}>
              ...
              <span style={{ color: '#64748b' }}>({step.role})</span>
            </div>)}
          </div>}
        </div>
      </div>}
      ...
      {tab === 'instances' && <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
          ...
          <tbody>{instances?.map(inst => <tr key={inst.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
            ...
            <td style={{ padding: '0.6rem', color: '#64748b' }}>{inst.entityType} / {inst.entityId.slice(0, 8)}</td>
            <td style={{ padding: '0.6rem', textAlign: 'center' }}><span style={{ background: inst.status === 'APPROVED' ? '#dcfce7' : inst.status === 'REJECTED' ? '#fef2f2' : '#fefce8', color: inst.status === 'APPROVED' ? '#16a34a' : inst.status === 'REJECTED' ? '#dc2626' : '#ca8a04', padding: '0.1rem 0.4rem', borderRadius: '999px' }}>{inst.status}</span></td>
