  ...
  return (
    <div>
      ...
      {showForm && <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', padding: '1rem', background: 'var(--bg)', borderRadius: '8px', border: '1px solid #e2e8f0', alignItems: 'flex-end' }}>
        ...
        <div style={{ width: '120px' }}><div style={{ fontSize: '0.8rem', color: 'var(--muted)' }}>Grade</div><input style={s.input} value={form.grade} onChange={e => setForm(f => ({ ...f, grade: e.target.value }))} /></div>
      ...
      <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr style={{ background: 'var(--bg)', borderBottom: '2px solid #e2e8f0' }}><th style={{ padding: '0.75rem', textAlign: 'left' }}>Title</th><th style={{ padding: '0.75rem', textAlign: 'left' }}>Org Unit</th><th style={{ padding: '0.75rem', textAlign: 'center' }}>Grade</th><th style={{ padding: '0.75rem', textAlign: 'center' }}>Status</th></tr></thead>
          <tbody>{positions?.map(p => <tr key={p.id} style={{ borderBottom: '1px solid #f1f5f9' }}><td style={{ padding: '0.75rem', fontWeight: 500 }}>{p.title}</td><td style={{ padding: '0.75rem', color: 'var(--muted)' }}>{p.orgUnitName || '-'}</td><td style={{ padding: '0.75rem', textAlign: 'center' }}>{p.grade || '-'}</td><td style={{ padding: '0.75rem', textAlign: 'center' }}><span style={{ background: p.isActive ? 'var(--success-soft)' : 'var(--danger-soft)', color: p.isActive ? 'var(--success)' : 'var(--danger)', padding: '0.15rem 0.5rem', borderRadius: '999px', fontSize: '0.8rem' }}>{p.isActive ? 'Active' : 'Inactive'}</span></td></tr>)}</tbody>
        </table>
      </div>
    </div>
  );
