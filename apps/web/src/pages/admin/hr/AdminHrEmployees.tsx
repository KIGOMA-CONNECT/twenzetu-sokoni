  ...
  return (
    <div>
      ...
      {showForm && <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '0.5rem', marginBottom: '1rem', padding: '1rem', background: 'var(--bg)', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
        ...
        <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Last Name *</div><input style={s.input} value={f.lastName} onChange={e => setF(p => ({ ...p, lastName: e.target.value }))} /></div>
        <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Email</div><input style={s.input} value={f.email} onChange={e => setF(p => ({ ...p, email: e.target.value }))} /></div>
        <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Phone</div><input style={s.input} value={f.phoneNumber} onChange={e => setF(p => ({ ...p, phoneNumber: e.target.value }))} /></div>
        <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Position</div><select style={s.sel} value={f.positionId} onChange={e => setF(p => ({ ...p, positionId: e.target.value }))}><option value="">Select...</option>{positions?.map(p => <option key={p.id} value={p.id}>{p.title}</option>)}</select></div>
        <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Org Unit</div><select style={s.sel} value={f.orgUnitId} onChange={e => setF(p => ({ ...p, orgUnitId: e.target.value }))}><option value="">Select...</option>{flatten(orgUnits).map(u => <option key={u.id} value={u.id}>{u.name}</option>)}</select></div>
        <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Hire Date</div><input type="date" style={s.input} value={f.hireDate} onChange={e => setF(p => ({ ...p, hireDate: e.target.value }))} /></div>
        <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Type</div><select style={s.sel} value={f.employmentType} onChange={e => setF(p => ({ ...p, employmentType: e.target.value }))}><option value="FULL_TIME">Full Time</option><option value="PART_TIME">Part Time</option><option value="CONTRACT">Contract</option><option value="INTERN">Intern</option></select></div>
      ...
      <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr style={{ background: 'var(--bg)', borderBottom: '2px solid #e2e8f0' }}><th style={{ padding: '0.75rem', textAlign: 'left' }}>Code</th><th style={{ padding: '0.75rem', textAlign: 'left' }}>Name</th><th style={{ padding: '0.75rem', textAlign: 'left' }}>Position</th><th style={{ padding: '0.75rem', textAlign: 'left' }}>Org Unit</th><th style={{ padding: '0.75rem', textAlign: 'left' }}>Email</th><th style={{ padding: '0.75rem', textAlign: 'center' }}>Status</th><th style={{ padding: '0.75rem', textAlign: 'center' }}>Actions</th></tr></thead>
