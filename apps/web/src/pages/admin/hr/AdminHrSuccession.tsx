  ...
  return (
    <div>
      ...
      {showForm && <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', padding: '1rem', background: '#f8fafc', borderRadius: '8px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
        <div><div style={{ fontSize: '0.75rem', color: '#64748b' }}>Position *</div><select style={s.input} value={f.positionId} onChange={e => setF(p => ({ ...p, positionId: e.target.value }))}><option value="">Select...</option>{positions?.map(p => <option key={p.id} value={p.id}>{p.title}</option>)}</select></div>
        <div><div style={{ fontSize: '0.75rem', color: '#64748b' }}>Risk</div><select style={s.input} value={f.riskLevel} onChange={e => setF(p => ({ ...p, riskLevel: e.target.value }))}><option value="LOW">Low</option><option value="MEDIUM">Medium</option><option value="HIGH">High</option><option value="CRITICAL">Critical</option></select></div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
        <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead><tr style={{ background: '#f8fafc' }}><th style={{ padding: '0.6rem', textAlign: 'left' }}>Position</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Risk</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Status</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Actions</th></tr></thead>
            <tbody>{plans?.map(p => <tr key={p.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
              ...
              <td style={{ padding: '0.6rem', textAlign: 'center' }}><span style={{ background: p.riskLevel === 'CRITICAL' ? '#fef2f2' : p.riskLevel === 'HIGH' ? '#fff7ed' : '#f0fdf4', color: p.riskLevel === 'CRITICAL' ? '#dc2626' : p.riskLevel === 'HIGH' ? '#ea580c' : '#16a34a', padding: '0.1rem 0.4rem', borderRadius: '999px', fontWeight: 600 }}>{p.riskLevel}</span></td>
              <td style={{ padding: '0.6rem', textAlign: 'center' }}>{p.status}</td>
              <td style={{ padding: '0.6rem', textAlign: 'center' }}>{p.status === 'ACTIVE' && <><button style={{ ...s.btn, background: '#3b82f6', marginRight: '0.3rem' }} onClick={() => setSelPlanId(p.id)}>Candidates</button><button style={{ ...s.btn, background: '#f59e0b' }} onClick={() => closePlan(p.id)}>Close</button></>}</td>
            </tr>)}</tbody>
          </table>
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '1rem' }}>
          ...
          {selPlanId ? <>
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.75rem', alignItems: 'flex-end', flexWrap: 'wrap' }}>
              <div><div style={{ fontSize: '0.75rem', color: '#64748b' }}>Employee</div><select style={s.input} value={candForm.employeeId} onChange={e => setCandForm(f => ({ ...f, employeeId: e.target.value }))}><option value="">Select...</option>{employees?.map(e => <option key={e.id} value={e.id}>{e.firstName} {e.lastName}</option>)}</select></div>
              <div><div style={{ fontSize: '0.75rem', color: '#64748b' }}>Readiness</div><select style={s.input} value={candForm.readinessLevel} onChange={e => setCandForm(f => ({ ...f, readinessLevel: e.target.value }))}><option value="READY_NOW">Ready Now</option><option value="READY_1_2_YEARS">1-2 Years</option><option value="READY_3_5_YEARS">3-5 Years</option><option value="POTENTIAL">Potential</option></select></div>
              <button style={{ ...s.btn, background: '#10b981' }} disabled={!candForm.employeeId} onClick={addCandidate}>Add</button>
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
              <thead><tr style={{ background: '#f8fafc' }}><th style={{ padding: '0.5rem', textAlign: 'left' }}>Employee</th><th style={{ padding: '0.5rem', textAlign: 'center' }}>Readiness</th><th style={{ padding: '0.5rem', textAlign: 'center' }}>Actions</th></tr></thead>
          ...
          </> : <div style={{ color: '#94a3b8', padding: '1rem', textAlign: 'center' }}>Select a plan to view candidates</div>}
        </div>
      </div>
    </div>
  );
