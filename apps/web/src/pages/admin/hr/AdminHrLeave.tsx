  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.4rem', marginBottom: '1rem' }}>
        ...
        <button onClick={() => setTab('requests')} style={{ padding: '0.4rem 0.8rem', borderRadius: '5px', border: tab === 'requests' ? '2px solid #3b82f6' : '1px solid #cbd5e1', background: tab === 'requests' ? 'var(--info-soft)' : '#fff', fontWeight: 500, color: tab === 'requests' ? '#3b82f6' : 'var(--muted)' }}>{t('hr.leave.leaveRequests')}</button>
      </div>
      ...
      {tab === 'types' && <div>
        <button style={{ ...s.btn, background: '#3b82f6', marginBottom: '1rem' }} onClick={() => setShowTypeForm(!showTypeForm)}>{showTypeForm ? t('hr.leave.cancel') : t('hr.leave.newLeaveType')}</button>
        {showTypeForm && <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', padding: '1rem', background: 'var(--bg)', borderRadius: '8px', alignItems: 'flex-end' }}>
          <div style={{ flex: 1 }}><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>{t('hr.leave.name')}</div><input style={s.input} value={tf.name} onChange={e => setTf(f => ({ ...f, name: e.target.value }))} /></div>
          <div style={{ width: '100px' }}><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>{t('hr.leave.days')}</div><input type="number" style={s.input} value={tf.defaultDays} onChange={e => setTf(f => ({ ...f, defaultDays: +e.target.value }))} /></div>
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.85rem' }}><input type="checkbox" checked={tf.isPaid} onChange={e => setTf(f => ({ ...f, isPaid: e.target.checked }))} /> {t('hr.leave.paid')}</label>
          <button style={{ ...s.btn, background: '#10b981' }} disabled={saving || !tf.name.trim()} onClick={createType}>{saving ? t('hr.leave.saving') : t('hr.leave.create')}</button>
        </div>}
        <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead><tr style={{ background: 'var(--bg)' }}><th style={{ padding: '0.6rem', textAlign: 'left', borderBottom: '1px solid #e2e8f0' }}>{t('hr.leave.colName')}</th><th style={{ padding: '0.6rem', textAlign: 'center', borderBottom: '1px solid #e2e8f0' }}>{t('hr.leave.colDefaultDays')}</th><th style={{ padding: '0.6rem', textAlign: 'center', borderBottom: '1px solid #e2e8f0' }}>{t('hr.leave.colPaid')}</th><th style={{ padding: '0.6rem', textAlign: 'center', borderBottom: '1px solid #e2e8f0' }}>{t('hr.leave.colStatus')}</th></tr></thead>
            <tbody>{types?.map(ty => <tr key={ty.id} style={{ borderBottom: '1px solid #f1f5f9' }}><td style={{ padding: '0.6rem', fontWeight: 500 }}>{ty.name}</td><td style={{ padding: '0.6rem', textAlign: 'center' }}>{ty.defaultDays}</td><td style={{ padding: '0.6rem', textAlign: 'center' }}>{ty.isPaid ? '✓' : '✗'}</td><td style={{ padding: '0.6rem', textAlign: 'center' }}><span style={{ background: ty.isActive ? 'var(--success-soft)' : 'var(--danger-soft)', color: ty.isActive ? 'var(--success)' : 'var(--danger)', padding: '0.1rem 0.4rem', borderRadius: '999px' }}>{ty.isActive ? t('hr.leave.active') : t('hr.leave.inactive')}</span></td></tr>)}</tbody>
          </table>
        </div>
      </div>}
      ...
      {tab === 'requests' && <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
          <thead><tr style={{ background: 'var(--bg)' }}><th style={{ padding: '0.6rem', textAlign: 'left', borderBottom: '1px solid #e2e8f0' }}>{t('hr.leave.colEmployee')}</th><th style={{ padding: '0.6rem', textAlign: 'left', borderBottom: '1px solid #e2e8f0' }}>{t('hr.leave.colType')}</th><th style={{ padding: '0.6rem', textAlign: 'left', borderBottom: '1px solid #e2e8f0' }}>{t('hr.leave.colDates')}</th><th style={{ padding: '0.6rem', textAlign: 'center', borderBottom: '1px solid #e2e8f0' }}>{t('hr.leave.colDays')}</th><th style={{ padding: '0.6rem', textAlign: 'center', borderBottom: '1px solid #e2e8f0' }}>{t('hr.leave.colStatus')}</th><th style={{ padding: '0.6rem', textAlign: 'center', borderBottom: '1px solid #e2e8f0' }}>{t('hr.leave.colActions')}</th></tr></thead>
          <tbody>{requests?.map(r => <tr key={r.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
            ...
            <td style={{ padding: '0.6rem', textAlign: 'center' }}>
              {r.status === 'PENDING' && <div style={{ display: 'flex', gap: '0.3rem', justifyContent: 'center' }}>
                <button style={{ ...s.btn, background: '#10b981' }} onClick={() => approveReq(r.id)}>{t('hr.leave.approve')}</button>
                <button style={{ ...s.btn, background: '#ef4444' }} onClick={() => rejectReq(r.id)}>{t('hr.leave.reject')}</button>
              </div>}
              {r.status === 'APPROVED' && <button style={{ ...s.btn, background: 'var(--muted)' }} onClick={() => cancelReq(r.id)}>{t('hr.leave.cancelAction')}</button>}
            </td>
          </tr>)}</tbody>
        </table>
      </div>}
