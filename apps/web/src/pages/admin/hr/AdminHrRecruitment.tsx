  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '0.4rem', marginBottom: '1rem' }}>
        {[['requisitions', t('hr.recruitment.tabRequisitions')], ['candidates', t('hr.recruitment.tabCandidates')], ['applications', t('hr.recruitment.tabApplications')]].map(([key, label]) => <button key={key} onClick={() => setTab(key as any)} style={{ padding: '0.4rem 0.8rem', borderRadius: '5px', border: tab === key ? '2px solid #3b82f6' : '1px solid #cbd5e1', background: tab === key ? 'var(--info-soft)' : '#fff', fontWeight: 500, color: tab === key ? '#3b82f6' : 'var(--muted)', textTransform: 'capitalize' }}>{label}</button>)}
      </div>
