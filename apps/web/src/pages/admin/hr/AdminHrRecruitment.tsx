    ...
    <div>
      ...
      {tab === 'requisitions' && <div>
        ...
        {showReqForm && <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.5rem', marginBottom: '1rem', padding: '1rem', background: 'var(--bg)', borderRadius: '8px' }}>
          <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Title *</div><input style={s.input} value={rf.title} onChange={e => setRf(f => ({ ...f, title: e.target.value }))} /></div>
          <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Vacancies</div><input type="number" style={s.input} value={rf.vacancies} onChange={e => setRf(f => ({ ...f, vacancies: +e.target.value }))} /></div>
          <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Description</div><input style={s.input} value={rf.description} onChange={e => setRf(f => ({ ...f, description: e.target.value }))} /></div>
        ...
        <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead><tr style={{ background: 'var(--bg)' }}><th style={{ padding: '0.6rem', textAlign: 'left' }}>Title</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Vacancies</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Status</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Actions</th></tr></thead>
      ...
      {tab === 'candidates' && <div>
        ...
        {showCandForm && <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', padding: '1rem', background: 'var(--bg)', borderRadius: '8px', alignItems: 'flex-end' }}>
          <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>First *</div><input style={{ ...s.input, width: '150px' }} value={cf.firstName} onChange={e => setCf(f => ({ ...f, firstName: e.target.value }))} /></div>
          <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Last *</div><input style={{ ...s.input, width: '150px' }} value={cf.lastName} onChange={e => setCf(f => ({ ...f, lastName: e.target.value }))} /></div>
          <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Email</div><input style={{ ...s.input, width: '200px' }} value={cf.email} onChange={e => setCf(f => ({ ...f, email: e.target.value }))} /></div>
          <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Phone</div><input style={{ ...s.input, width: '150px' }} value={cf.phoneNumber} onChange={e => setCf(f => ({ ...f, phoneNumber: e.target.value }))} /></div>
        ...
        <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead><tr style={{ background: 'var(--bg)' }}><th style={{ padding: '0.6rem', textAlign: 'left' }}>Name</th><th style={{ padding: '0.6rem', textAlign: 'left' }}>Email</th><th style={{ padding: '0.6rem', textAlign: 'left' }}>Phone</th><th style={{ padding: '0.6rem', textAlign: 'left' }}>Source</th></tr></thead>
      ...
      {tab === 'applications' && <div>
        ...
        {showAppForm && <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', padding: '1rem', background: 'var(--bg)', borderRadius: '8px', alignItems: 'flex-end' }}>
          <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Candidate</div><select style={s.sel} value={af.candidateId} onChange={e => setAf(f => ({ ...f, candidateId: e.target.value }))}><option value="">Select...</option>{candidates?.map(c => <option key={c.id} value={c.id}>{c.firstName} {c.lastName}</option>)}</select></div>
          <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Requisition</div><select style={s.sel} value={af.requisitionId} onChange={e => setAf(f => ({ ...f, requisitionId: e.target.value }))}><option value="">Select...</option>{reqs?.map(r => <option key={r.id} value={r.id}>{r.title}</option>)}</select></div>
          <button style={{ ...s.btn, background: '#10b981' }} disabled={!af.candidateId || !af.requisitionId} onClick={createApp}>Create</button>
        </div>}
        <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead><tr style={{ background: 'var(--bg)' }}><th style={{ padding: '0.6rem', textAlign: 'left' }}>Candidate</th><th style={{ padding: '0.6rem', textAlign: 'left' }}>Requisition</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Stage</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Actions</th></tr></thead>
