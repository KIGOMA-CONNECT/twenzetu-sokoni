...
export default function AdminOrgTypes() {
  ...
  const create = async () => { if (!name.trim()) return; setSaving(true); try { await api.post('/organization/types', { name, description: desc }); setName(''); setDesc(''); refetch(); } catch { /* no-op */} finally { setSaving(false); } };
