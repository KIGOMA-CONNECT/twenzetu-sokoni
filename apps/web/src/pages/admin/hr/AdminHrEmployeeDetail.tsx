...
export default function AdminHrEmployeeDetail() {
  ...
  if (!emp) return <div style={{ color: 'var(--muted)' }}>Employee not found</div>;
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
        <div>
          ...
          <div style={{ color: 'var(--muted)', fontSize: '0.9rem' }}>{emp.positionTitle || 'No position'} · {emp.employeeCode} · <span style={{ background: emp.status === 'ACTIVE' ? 'var(--success-soft)' : emp.status === 'SUSPENDED' ? 'var(--danger-soft)' : 'var(--line-soft)', color: emp.status === 'ACTIVE' ? 'var(--success)' : emp.status === 'SUSPENDED' ? 'var(--danger)' : 'var(--muted)', padding: '0.1rem 0.4rem', borderRadius: '999px', fontSize: '0.8rem' }}>{emp.status}</span></div>
        </div>
        <div style={{ display: 'flex', gap: '0.4rem' }}>
          {emp.status === 'ACTIVE' && <><button style={{ ...s.btnSm, background: 'var(--warning)' }} onClick={suspend}>Suspend</button><button style={{ ...s.btnSm, background: 'var(--danger)' }} onClick={terminate}>Terminate</button></>}
      ...
      {tab === 'overview' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '1rem' }}>
          <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '1rem' }}>
            ...
            <div style={{ fontSize: '0.85rem', color: 'var(--muted)', lineHeight: 1.8 }}>Type: {emp.employmentType || 'N/A'}<br />Hired: {new Date(emp.hireDate).toLocaleDateString()}<br />Org: {emp.orgUnitName || '-'}<br />Code: {emp.employeeCode}</div>
          </div>
          <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '1rem' }}>
            <div style={{ fontWeight: 600, marginBottom: '0.5rem' }}>Contact</div>
            <div style={{ fontSize: '0.85rem', color: 'var(--muted)', lineHeight: 1.8 }}>Email: {emp.email || '-'}<br />Phone: {emp.phoneNumber || '-'}<br />Work Email: {emp.workEmail || '-'}</div>
          </div>
          <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '1rem' }}>
            <div style={{ fontWeight: 600, marginBottom: '0.5rem' }}>Leave Balances</div>
            {balances?.length ? balances.map(b => <div key={b.id} style={{ fontSize: '0.85rem', display: 'flex', justifyContent: 'space-between', padding: '0.2rem 0' }}><span>{b.leaveTypeName}</span><span style={{ fontWeight: 600 }}>{b.remainingDays}d</span></div>) : <div style={{ fontSize: '0.85rem', color: 'var(--faint)' }}>No balances set</div>}
          </div>
        </div>
      )}
        ...
        <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '1.5rem' }}>
          {editDetails ? (
            <div>
              ...
              <button style={{ ...s.btnSm, background: 'var(--muted)', marginLeft: '0.5rem' }} onClick={() => setEditDetails(false)}>Cancel</button>
            </div>
          ) : (
            <div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '0.75rem', marginBottom: '1rem' }}>
      ...
      {tab === 'leave' && (
        <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '1.5rem' }}>
          ...
          {requests?.length ? <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead><tr style={{ background: 'var(--bg)' }}><th style={{ padding: '0.5rem', textAlign: 'left', borderBottom: '1px solid #e2e8f0' }}>Type</th><th style={{ padding: '0.5rem', textAlign: 'left', borderBottom: '1px solid #e2e8f0' }}>Dates</th><th style={{ padding: '0.5rem', textAlign: 'center', borderBottom: '1px solid #e2e8f0' }}>Days</th><th style={{ padding: '0.5rem', textAlign: 'center', borderBottom: '1px solid #e2e8f0' }}>Status</th></tr></thead>
          ...
          </table> : <div style={{ color: 'var(--faint)' }}>No leave requests</div>}
        </div>
      )}
      ...
      {tab === 'attendance' && (
        <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '1.5rem' }}>
          ...
          {attendance?.length ? <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead><tr style={{ background: 'var(--bg)' }}><th style={{ padding: '0.5rem', textAlign: 'left', borderBottom: '1px solid #e2e8f0' }}>Date</th><th style={{ padding: '0.5rem', textAlign: 'left', borderBottom: '1px solid #e2e8f0' }}>Clock In</th><th style={{ padding: '0.5rem', textAlign: 'left', borderBottom: '1px solid #e2e8f0' }}>Clock Out</th><th style={{ padding: '0.5rem', textAlign: 'center', borderBottom: '1px solid #e2e8f0' }}>Hours</th></tr></thead>
          ...
          </table> : <div style={{ color: 'var(--faint)' }}>No attendance records</div>}
        </div>
      )}
      ...
      {tab === 'documents' && (
        <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '1.5rem' }}>
          ...
          {documents?.length ? <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead><tr style={{ background: 'var(--bg)' }}><th style={{ padding: '0.5rem', textAlign: 'left', borderBottom: '1px solid #e2e8f0' }}>Type</th><th style={{ padding: '0.5rem', textAlign: 'left', borderBottom: '1px solid #e2e8f0' }}>File</th><th style={{ padding: '0.5rem', textAlign: 'right', borderBottom: '1px solid #e2e8f0' }}>Uploaded</th></tr></thead>
          ...
          </table> : <div style={{ color: 'var(--faint)' }}>No documents</div>}
        </div>
      )}
    </div>
  );
