  ...
  return (
    <div>
      ...
      {showForm && <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', padding: '1rem', background: 'var(--bg)', borderRadius: '8px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
        ...
        <div style={{ width: '150px' }}><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Provider</div><input style={s.input} value={f.provider} onChange={e => setF(p => ({ ...p, provider: e.target.value }))} /></div>
        ...
        <div>
          <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
              <thead><tr style={{ background: 'var(--bg)' }}><th style={{ padding: '0.6rem', textAlign: 'left' }}>Course</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Hours</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Status</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Actions</th></tr></thead>
              <tbody>{courses?.map(c => <tr key={c.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                ...
                <td style={{ padding: '0.6rem', textAlign: 'center' }}><span style={{ background: c.isActive ? 'var(--success-soft)' : 'var(--danger-soft)', color: c.isActive ? 'var(--success)' : 'var(--danger)', padding: '0.1rem 0.4rem', borderRadius: '999px' }}>{c.isActive ? 'Active' : 'Inactive'}</span></td>
        ...
        <div>
          <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.75rem', alignItems: 'flex-end' }}>
              ...
              <div><div style={{ fontSize: '0.75rem', color: 'var(--muted)' }}>Course</div><select style={s.input} value={enrollCourseId} onChange={e => setEnrollCourseId(e.target.value)}><option value="">Select...</option>{courses?.filter(c => c.isActive).map(c => <option key={c.id} value={c.id}>{c.title}</option>)}</select></div>
            ...
            {selEmpId && enrollments && <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
              <thead><tr style={{ background: 'var(--bg)' }}><th style={{ padding: '0.5rem', textAlign: 'left' }}>Course</th><th style={{ padding: '0.5rem', textAlign: 'center' }}>Status</th><th style={{ padding: '0.5rem', textAlign: 'center' }}>Actions</th></tr></thead>
              <tbody>{enrollments.map(e => <tr key={e.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                ...
                <td style={{ padding: '0.5rem', textAlign: 'center' }}>
                  {e.status === 'ENROLLED' && <><button style={{ ...s.btn, background: '#10b981', marginRight: '0.3rem' }} onClick={() => completeEnroll(e.id)}>Complete</button><button style={{ ...s.btn, background: 'var(--warning)' }} onClick={() => cancelEnroll(e.id)}>Cancel</button></>}
                </td>
              </tr>)}</tbody>
            </table>}
          </div>
