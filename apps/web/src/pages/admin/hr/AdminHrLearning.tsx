  ...
  return (
    <div>
      ...
      {showForm && <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', padding: '1rem', background: '#f8fafc', borderRadius: '8px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
        <div style={{ width: '200px' }}><div style={{ fontSize: '0.75rem', color: '#64748b' }}>Title *</div><input style={s.input} value={f.title} onChange={e => setF(p => ({ ...p, title: e.target.value }))} /></div>
        <div style={{ width: '100px' }}><div style={{ fontSize: '0.75rem', color: '#64748b' }}>Hours</div><input type="number" style={s.input} value={f.durationHours} onChange={e => setF(p => ({ ...p, durationHours: +e.target.value }))} /></div>
        <div style={{ width: '150px' }}><div style={{ fontSize: '0.75rem', color: '#64748b' }}>Provider</div><input style={s.input} value={f.provider} onChange={e => setF(p => ({ ...p, provider: e.target.value }))} /></div>
        <button style={{ ...s.btn, background: '#10b981' }} disabled={saving || !f.title.trim()} onClick={create}>{saving ? 'Saving...' : 'Create'}</button>
      </div>}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
        <div>
          <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
              <thead><tr style={{ background: '#f8fafc' }}><th style={{ padding: '0.6rem', textAlign: 'left' }}>Course</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Hours</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Status</th><th style={{ padding: '0.6rem', textAlign: 'center' }}>Actions</th></tr></thead>
              <tbody>{courses?.map(c => <tr key={c.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                ...
                <td style={{ padding: '0.6rem', textAlign: 'center' }}><span style={{ background: c.isActive ? '#dcfce7' : '#fef2f2', color: c.isActive ? '#16a34a' : '#dc2626', padding: '0.1rem 0.4rem', borderRadius: '999px' }}>{c.isActive ? 'Active' : 'Inactive'}</span></td>
                <td style={{ padding: '0.6rem', textAlign: 'center' }}>{c.isActive && <button style={{ ...s.btn, background: '#ef4444' }} onClick={() => deactivate(c.id)}>Deactivate</button>}</td>
              </tr>)}</tbody>
            </table>
          </div>
        </div>
        <div>
          <div style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e2e8f0', padding: '1rem' }}>
            <div style={{ fontWeight: 600, marginBottom: '0.75rem' }}>Employee Enrollments</div>
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.75rem', alignItems: 'flex-end' }}>
              ...
              <div><div style={{ fontSize: '0.75rem', color: '#64748b' }}>Course</div><select style={s.input} value={enrollCourseId} onChange={e => setEnrollCourseId(e.target.value)}><option value="">Select...</option>{courses?.filter(c => c.isActive).map(c => <option key={c.id} value={c.id}>{c.title}</option>)}</select></div>
              <button style={{ ...s.btn, background: '#3b82f6' }} disabled={!selEmpId || !enrollCourseId} onClick={enroll}>Enroll</button>
            </div>
            {selEmpId && enrollments && <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
              <thead><tr style={{ background: '#f8fafc' }}><th style={{ padding: '0.5rem', textAlign: 'left' }}>Course</th><th style={{ padding: '0.5rem', textAlign: 'center' }}>Status</th><th style={{ padding: '0.5rem', textAlign: 'center' }}>Actions</th></tr></thead>
              <tbody>{enrollments.map(e => <tr key={e.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                ...
                <td style={{ padding: '0.5rem', textAlign: 'center' }}><span style={{ background: e.status === 'COMPLETED' ? '#dcfce7' : e.status === 'ENROLLED' ? '#fefce8' : '#f1f5f9', color: e.status === 'COMPLETED' ? '#16a34a' : e.status === 'ENROLLED' ? '#ca8a04' : '#64748b', padding: '0.1rem 0.4rem', borderRadius: '999px' }}>{e.status}</span></td>
                <td style={{ padding: '0.5rem', textAlign: 'center' }}>
                  {e.status === 'ENROLLED' && <><button style={{ ...s.btn, background: '#10b981', marginRight: '0.3rem' }} onClick={() => completeEnroll(e.id)}>Complete</button><button style={{ ...s.btn, background: '#f59e0b' }} onClick={() => cancelEnroll(e.id)}>Cancel</button></>}
                </td>
              </tr>)}</tbody>
            </table>}
          </div>
