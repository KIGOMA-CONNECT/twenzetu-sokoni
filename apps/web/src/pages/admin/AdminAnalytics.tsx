...
export default function AdminAnalytics() {
  ...
  const inventoryQuery = `/admin/analytics/inventory?threshold=${threshold}`;
  ...
  const exportCsv = () => {
    ...
    if (tab === 'overview' && overview) {
      ...
      downloadCsv(`platform-analytics-overview-${period}-${stamp}.csv`, [
        ['Metric', 'Value'],
      ...
      downloadCsv(`platform-analytics-top-products-${period}-${stamp}.csv`, [
      ...
      downloadCsv(`platform-analytics-inventory-${stamp}.csv`, [
    ...
    } else if (tab === 'disputes' && disputes) {
      downloadCsv(`platform-analytics-disputes-${stamp}.csv`, [
        ['Metric', 'Value'],
        ['Total Disputes', disputes.total],
        ['Open', disputes.open],
    ...
    <div style={styles.container}>
      <div style={styles.header}>
        <div>
          ...
          <div style={styles.subtitle}>Tenant-wide reports with the defined metric catalog</div>
        </div>
      ...
      {tab === 'disputes' && (disputesLoading ? (
        <LoadingSpinner />
      ) : disputesError ? (
        <ErrorMessage message={disputesError} />
      ) : !disputes ? (
        <div style={styles.empty}>No dispute metrics</div>
      ) : (
        <>
          <div style={styles.cards}>
            {renderStatCard('Total Disputes', String(disputes.total))}
            {renderStatCard('Open', String(disputes.open))}
            {renderStatCard('Resolved', String(disputes.resolved))}
            {renderStatCard('Avg Resolution', `${fmt(disputes.averageResolutionTimeHours)} h`)}
            {renderStatCard('Resolve Rate', `${disputes.total > 0 ? Math.round((disputes.resolved / disputes.total) * 100) : 0}%`)}
          </div>
          {Object.keys(disputes.byReason).length > 0 && (
            <div style={styles.panel}>
              <div style={styles.panelHeader}>
                <span>By Reason</span>
              </div>
              <table style={styles.table}>
                ...
                <tbody>
                  {Object.entries(disputes.byReason).map(([reason, count]) => (
                    <tr key={reason}>
                      <td style={styles.td}>{reason.replace(/_/g, ' ')}</td>
          ...
          {Object.keys(disputes.bySeverity).length > 0 && (
            <div style={styles.panel}>
              <div style={styles.panelHeader}>
              ...
              <table style={styles.table}>
                ...
                <tbody>
                  {Object.entries(disputes.bySeverity).map(([severity, count]) => (
                    <tr key={severity}>
                      <td style={styles.td}>{severity}</td>
                      <td style={styles.tdRight}>{count}</td>
                    </tr>
                  ))}
                </tbody>
