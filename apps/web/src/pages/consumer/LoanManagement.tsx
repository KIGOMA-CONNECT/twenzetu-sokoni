...
export default function LoanManagement() {
  ...
  const views: Array<{ id: LoansView; label: string; show: boolean }> = [
    { id: 'catalog', label: t('loans.loanProducts'), show: true },
    { id: 'applied', label: t('loans.appliedTab', { count: applied.length }), show: true },
    { id: 'existing', label: t('loans.existingTab', { count: existing.length }), show: true },
  ];
  ...
  return (
    <div className="card" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      {view === 'applied' && (
        <div>
          <h3 className="section-title">{t('loans.appliedLoans')}</h3>
          {loading ? (
            <LoadingSpinner />
          ) : error ? (
            <ErrorMessage message={error} />
          ) : applied.length === 0 ? (
            <EmptyState icon="📝" title={t('loans.noAppliedLoans')} sub={t('loans.noAppliedLoansSub')} />
          ) : (
            <div className="table-wrap">
              <table className="table">
                <thead>
                  <tr>
                    <th>{t('loans.applicationNo')}</th>
                    <th>{t('loans.fsp')}</th>
                    <th>{t('loans.requested')}</th>
                    <th>{t('loans.totalToPay')}</th>
                    <th>{t('loans.deductionPerMonth')}</th>
                ...
                <tbody>
                  {applied.map((loan) => (
                    <tr key={loan.id}>
                      ...
                      <td>
                        <button className="btn btn-outline btn-sm" onClick={() => setSelectedLoanId(loan.id)}>{t('loans.view')}</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {view === 'existing' && (
        <div>
          <h3 className="section-title">{t('loans.existingLoans')}</h3>
          {loading ? (
            <LoadingSpinner />
          ) : error ? (
            <ErrorMessage message={error} />
          ) : existing.length === 0 ? (
            <EmptyState icon="✅" title={t('loans.noExistingLoans')} sub={t('loans.noExistingLoansSub')} />
          ) : (
            <div className="table-wrap">
              <table className="table">
                <thead>
                  <tr>
                    <th>{t('loans.applicationNo')}</th>
                    <th>{t('loans.deductionCode')}</th>
                    <th>{t('loans.collector')}</th>
                    <th>{t('loans.principal')}</th>
                    <th>{t('loans.amountPerMonth')}</th>
                ...
                <tbody>
                  {existing.map((loan) => (
                    <tr key={loan.id}>
                      ...
                      <td>
                        <button className="btn btn-outline btn-sm" onClick={() => setSelectedLoanId(loan.id)}>{t('loans.manage')}</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
...
function CatalogSection({ borrowerType, onLoanCreated }: { borrowerType: string; onLoanCreated: () => void }) {
  ...
  if (eligible.length === 0) return <EmptyState icon="📚" title={t('loans.noLoanProducts')} sub={t('loans.checkBackSoon')} />;

  return (
    <div>
      <h3 className="section-title">{t('loans.loanProductCatalog')}</h3>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1rem' }}>
        {eligible.map((p) => (
          <div key={p.id} className="card" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            ...
            <button className="btn btn-primary" onClick={() => setSelected(p)}>{t('loans.apply')}</button>
          </div>
        ))}
      </div>
    </div>
...
function ApplyForm({ product, borrowerType, onCancel, onSubmitted }: {
  ...
  const handleUpload = async (type: string, file: File) => {
    ...
    try {
      ...
      setErr(e.response?.data?.message || e.response?.data?.error?.message || e.message || t('loans.uploadFailed'));
    } finally {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setErr(e.response?.data?.message || e.response?.data?.error?.message || e.message || t('loans.applyFailed'));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="card" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        ...
        <button className="btn btn-outline btn-sm" onClick={onCancel} disabled={busy}>{t('loans.backToCatalog')}</button>
      </div>
      ...
      {walletInfo && walletInfo.maxLoanAmount > 0 && (
        <div className="card" style={{ padding: '1rem', background: 'var(--brand-soft)', border: '1px solid var(--brand)', borderRadius: 'var(--radius)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '0.5rem' }}>
            <div>
              <div style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.03em' }}>💰 {t('loans.walletBalance')}</div>
              <div style={{ fontSize: '1.3rem', fontWeight: 800, color: 'var(--ink)' }}>{formatCurrency(walletInfo.walletBalance)}</div>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.03em' }}>{t('loans.maxLoan')}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1rem' }}>
          ...
          <label className="field-label">{t('loans.amountLabel', { min: formatCurrency(product.minAmount), max: formatCurrency(product.maxAmount) })}</label>
          ...
          <label className="field-label" style={{ marginTop: '0.5rem' }}>{t('loans.tenureLabel', { min: product.minTermMonths, max: product.maxTermMonths })}</label>
          ...
          <label className="field-label" style={{ marginTop: '0.5rem' }}>{t('loans.purposeLabel')}</label>
          <input type="text" className="input" value={purpose} onChange={(e) => setPurpose(e.target.value)} placeholder={t('loans.purposePlaceholder')} />
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', fontSize: '0.9rem' }}>
          <div style={{ fontWeight: 800, marginBottom: '0.25rem' }}>{t('loans.costBreakdown')}</div>
          <Row label={t('loans.netAmount')} value={formatCurrency(principal)} />
          <Row label={t('loans.interest')} value={formatCurrency(breakdown.interest)} />
          <Row label={t('loans.insurance')} value={formatCurrency(breakdown.insurance)} />
          <Row label={t('loans.processingFee')} value={formatCurrency(breakdown.processing)} />
          <Row label={t('loans.liquidationAmount')} value={formatCurrency(breakdown.liquidation)} />
          <div style={{ borderTop: '1px solid var(--line)', margin: '0.25rem 0' }} />
          <Row label={t('loans.totalToPay')} value={formatCurrency(breakdown.total)} bold />
          <Row label={t('loans.deductiblePerMonth')} value={formatCurrency(breakdown.deductible)} bold />
          <Row label={t('loans.installmentPerMonth')} value={formatCurrency(breakdown.monthly)} />
        </div>
      </div>

      <div>
        <div style={{ fontWeight: 800, marginBottom: '0.5rem' }}>{t('loans.attachmentsLabel', { status: missing.length > 0 ? `${missing.length} ${t('loans.requiredRemaining', { count: missing.length })}` : t('loans.completeLabel') })}</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          {allAttachments.map((a) => (
            <div key={a.type} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.75rem', flexWrap: 'wrap', padding: '0.6rem 0.75rem', borderRadius: 'var(--radius)', border: '1px solid var(--line)', background: 'var(--surface)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <span className={`badge ${a.required ? 'badge-amber' : ''}`}>{a.required ? t('loans.requiredLabel') : t('loans.optionalLabel')}</span>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <label className="btn btn-outline btn-sm" style={{ cursor: 'pointer', marginBottom: 0 }}>
                  {uploadingType === a.type ? t('loans.uploading') : files[a.type] ? t('loans.replace') : t('loans.upload')}
                  <input
                    type="file"
      ...
      >
        {missing.length > 0 ? t('loans.uploadRequired', { count: missing.length }) : t('loans.submitApplication')}
      </button>
    </div>
  );
}
...
function LoanDetailView({ loanId, onBack, onRefresh }: { loanId: string; onBack: () => void; onRefresh: () => void }) {
  ...
  const run = async (fn: () => Promise<unknown>, successMsg: string) => {
    ...
    try {
      ...
      setErr(e.response?.data?.message || e.response?.data?.error?.message || e.message || t('loans.actionFailed'));
    } finally {
      setBusy(false);
    }
  };
  ...
  if (loading) return <LoadingSpinner />;
  if (error) return <ErrorMessage message={error} />;
  if (!loan) return <ErrorMessage message={t('loans.loanNotFound')} />;
    ...
    <div className="card" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          ...
          <div style={{ fontSize: '0.85rem', color: 'var(--muted)' }}>
            {t('loans.applied')} {formatDate(loan.createdAt)} · {t('loans.status')}: <strong>{STATUS_LABEL[loan.status] ?? loan.status}</strong>
            {loan.rejectionReason && <span style={{ color: 'var(--danger)' }}> · {loan.rejectionReason}</span>}
          </div>
        </div>
        <button className="btn btn-outline btn-sm" onClick={onBack}>{t('loans.back')}</button>
      </div>

      {/* 5-step workflow timeline */}
      <div>
        <div style={{ fontWeight: 800, marginBottom: '0.5rem' }}>{t('loans.applicationWorkflow')}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1rem' }}>
        ...
        <div className="card" style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
          <div style={{ fontWeight: 800, marginBottom: '0.25rem' }}>{t('loans.amounts')}</div>
          <Row label={t('loans.requestedNet')} value={formatCurrency(loan.netAmount ?? loan.principal)} />
          <Row label={t('loans.interest')} value={formatCurrency(loan.interestAmount ?? 0)} />
          <Row label={t('loans.insurance')} value={formatCurrency(loan.insuranceAmount ?? 0)} />
          <Row label={t('loans.processingFee')} value={formatCurrency(loan.processingFeeAmount ?? 0)} />
          <Row label={t('loans.liquidationAmount')} value={formatCurrency(loan.liquidationAmount ?? 0)} />
          <div style={{ borderTop: '1px solid var(--line)', margin: '0.25rem 0' }} />
          <Row label={t('loans.totalToPay')} value={formatCurrency(loan.totalAmountToPay ?? loan.principal)} bold />
          <Row label={t('loans.deductiblePerMonth')} value={formatCurrency(loan.deductibleAmount ?? loan.monthlyPayment)} bold />
          <Row label={t('loans.tenure')} value={`${loan.termMonths} ${t('loans.tenureValue', { value: loan.termMonths })}`} />
          <Row label={t('loans.interestRate')} value={`${(loan.interestRate * 100).toFixed(1)}% / ${t('loans.fee')}`} />
        </div>

        <div className="card" style={{ padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
          <div style={{ fontWeight: 800, marginBottom: '0.25rem' }}>{t('loans.fspDetails')}</div>
          <Row label={t('loans.fspName')} value={loan.fspName ?? '-'} />
          <Row label={t('loans.fspCode')} value={loan.fspCode ?? '-'} />
          <Row label={t('loans.branch')} value={loan.branchName ?? '-'} />
          <Row label={t('loans.accountNumber')} value={loan.accountNumber ?? '-'} />
          <Row label={t('loans.repaymentCode')} value={loan.repaymentCode ?? '-'} />
          {loan.mobileNumber && <Row label={t('loans.mobile')} value={loan.mobileNumber} />}
          {loan.purpose && <Row label={t('loans.purpose')} value={loan.purpose} />}
        </div>
      </div>
      ...
      {active && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          <div style={{ fontWeight: 800 }}>{t('loans.actions')}</div>
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
            <div>
              <label className="field-label">{t('loans.repay')} (TZS)</label>
              <input type="number" className="input" value={repayAmt} min={1} onChange={(e) => setRepayAmt(e.target.value)} style={{ width: 140 }} />
            </div>
            <button className="btn btn-primary" disabled={busy || !repayAmt} onClick={() => run(() => api.post(`/fintech/loans/${loan.id}/repay`, { amount: Number(repayAmt) }), t('loans.repaymentRecorded'))}>{t('loans.repay')}</button>

            <div>
              <label className="field-label">{t('loans.topUp')} (TZS)</label>
              <input type="number" className="input" value={topUpAmt} min={1} onChange={(e) => setTopUpAmt(e.target.value)} style={{ width: 140 }} />
            </div>
            <button className="btn btn-outline" disabled={busy || !topUpAmt} onClick={() => run(() => api.post(`/fintech/loans/${loan.id}/topup`, { extraAmount: Number(topUpAmt) }), t('loans.topUpApplied'))}>{t('loans.topUp')}</button>

            <button className="btn btn-outline" disabled={busy} onClick={() => setRestructure((v) => !v)}>{t('loans.restructure')}</button>
            <button className="btn btn-outline" disabled={busy} onClick={() => setTakeover((v) => !v)}>{t('loans.takeover')}</button>
          </div>

          {restructure && (
            <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
              <div>
                <label className="field-label">{t('loans.newTerm')}</label>
              ...
              <button className="btn btn-primary" disabled={busy || !restructureTerm} onClick={() => run(() => api.post(`/fintech/loans/${loan.id}/restructure`, { newTermMonths: Number(restructureTerm) }), t('loans.loanRestructured'))}>{t('loans.confirmRestructure')}</button>
            </div>
          )}

          {takeover && (
            <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', alignItems: 'flex-end' }}>
              <div><label className="field-label">{t('loans.newFspName')}</label><input className="input" value={takeoverFsp} onChange={(e) => setTakeoverFsp(e.target.value)} style={{ width: 180 }} /></div>
              <div><label className="field-label">{t('loans.accountNumber')}</label><input className="input" value={takeoverAcct} onChange={(e) => setTakeoverAcct(e.target.value)} style={{ width: 160 }} /></div>
              <div><label className="field-label">{t('loans.deductionCode')}</label><input className="input" value={takeoverCode} onChange={(e) => setTakeoverCode(e.target.value)} style={{ width: 140 }} /></div>
              <button className="btn btn-primary" disabled={busy || !takeoverFsp || !takeoverAcct || !takeoverCode} onClick={() => run(() => api.post(`/fintech/loans/${loan.id}/takeover`, { fspName: takeoverFsp, accountNumber: takeoverAcct, repaymentCode: takeoverCode }), t('loans.loanTakenOver'))}>{t('loans.confirmTakeover')}</button>
            </div>
          )}
        </div>
      )}
      ...
      {/* Documents */}
      <div>
        <div style={{ fontWeight: 800, marginBottom: '0.5rem' }}>{t('loans.attachmentsTitle')}</div>
        {documents.length === 0 ? (
          <div style={{ fontSize: '0.85rem', color: 'var(--muted)' }}>{t('loans.noAttachments')}</div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
            {documents.map((d) => (
              <div key={d.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.75rem', padding: '0.5rem 0.75rem', borderRadius: 'var(--radius)', border: '1px solid var(--line)' }}>
                ...
                <a href={d.fileUrl} target="_blank" rel="noreferrer" className="btn btn-outline btn-sm">{t('loans.view')}</a>
              </div>
            ))}
          </div>
        )}
      ...
      <div>
        <div style={{ fontWeight: 800, marginBottom: '0.5rem' }}>{t('loans.amortizationSchedule', { months: loan.termMonths })}</div>
        {schedule.length === 0 ? (
          <div style={{ fontSize: '0.85rem', color: 'var(--muted)' }}>{t('loans.scheduleWillAppear')}</div>
        ) : (
      ...
      {data?.repayments && data.repayments.length > 0 && (
        <div>
          <div style={{ fontWeight: 800, marginBottom: '0.5rem' }}>{t('loans.repaymentHistory')}</div>
          <div className="table-wrap">
            <table className="table">
              <thead>
                <tr>
                    <th>{t('loans.date')}</th><th>{t('loans.amount')}</th><th>{t('loans.principal')}</th><th>{t('loans.interest')}</th><th>{t('loans.balance')}</th></tr>
              </thead>
              <tbody>
